...
/**
 * @param {String} node - The OWS target node
 * @param {String} service - The name of the service
 * @param {Function} done
 */
function startGulp(node, name, opts) {

  opts = opts || {};
  var browser = !opts.skipBrowser;
  var fullname = name ? node + '-' + name : node;
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/@wedontknowjs/ows-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = browserifyPath + ' --require ./index.js:' + fullname + ' --external ' + node + '-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = browserifyPath + ' --require ./index.js:' + node + '-lib -o ' + node + '-lib.js';
    }
